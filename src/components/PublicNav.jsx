				...
				<nav className='hidden lg:flex lg:items-center mt-[20px] p-2'>
					<ul className="flex justify-around items-center space-x-3 gap-4">
						{links.map((anchor, id) => (
							<li
								key={id}
								className={`text-center ${anchor.text} ${anchor.text}`}
							>
								<Link
									to={anchor.to}
									className='flex items-center text-2xl text-white font-medium hover:text-[#FAE7D5]'
								>
									<img
					...
					<nav className='bg-[#8BA8C4] w-3/4 max-w-xs h-full shadow-lg p-4 transition-transform transform translate-x-0 bg-opacity-90' onClick={(e) => e.stopPropagation()}>
						...
						<ul className="flex flex-col items-center gap-4 text-2xl ">
							{links.map((anchor, id) => (
								<li
									key={id}
									className={`text-center w-full text-center ${anchor.text} ${anchor.text}`}
								>
									<Link
										to={anchor.to}
										className='flex items-center text-white font-medium hover:text-[#FAE7D5]'
